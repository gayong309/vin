...
async def start(update: Update, context: CallbackContext):
    ...
    message = (
        "*🔥 Welcome to DDoS Bot 🔥*\n"
        "*🔥 Use /attack <ip:port> <time> to launch an attack*"
    )
...
async def attack(update: Update, context: CallbackContext):
    ...
    await context.bot.send_message(chat_id=chat_id, text=f"*✅ Attack started on {ip}:{port} for {time} seconds!*", parse_mode="Markdown")
...
async def stop(update: Update, context: CallbackContext):
    ...
    try:
        ...
        await asyncio.sleep(1)  # Allow time for termination
        attack_process.kill()
